...
contract RedsoftContract is ERC721URIStorage {
  ...
  event NFTListedEvent(uint256 price, uint256 indexed tokenId, address indexed oldOwnerOfnft);
  event NFTSoldEvent(
    uint256 price,
    uint256 indexed tokenId,
  ...
  function mintToken(string memory tokenURI) public returns (uint256) {
    ...
    emit NftMintedEvent(newTokenId, msg.sender, address(0), false, 0, false);
    return newTokenId;
  }
  ...
  function listNft(uint256 tokenId, uint256 price) public payable {
    ...
    require(tokenIdToNftObject[tokenId].isListed == false, 'Token is already listed');
    require(msg.value == listingPrice, 'You have to pay the listing price to mint');
  ...
  function fetchNFTsMintedUser(address user) public view returns (Nft[] memory) {
  ...
  function fetchNFTsListedUser(address user) public view returns (Nft[] memory) {
  ...
  function getUnsoldNFTsForUser(address user) public view returns (uint256[] memory) {
    uint256 totalNFTs = _tokenIds.current();
    uint256 unsoldNFTsCount = 0;
    uint256[] memory unsoldNFTs = new uint256[](totalNFTs);
    for (uint256 i = 1; i <= totalNFTs; i++) {
